...
void processImage_surface(const cv::Mat& depthImage,cv::Mat gray, int half_window_size, int depth_threshold, int intensity_threshold, int window_size, std::vector<cv::Point>& planePixels) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        threads.emplace_back(processImageRegions_surface, std::cref(depthImage),gray, startY, endY, half_window_size, depth_threshold, intensity_threshold, window_size, std::ref(planePixelsList[i]));
        startY = endY;
    }
...
void LaserProcessingClass::pointcloudtodepth(pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in,
    ...
    // 濾波後提取特徵
    GaussianBlur(gray, result_g, cv::Size(3,3), 0); // 高斯模糊
    orb->detectAndCompute(result_g, cv::noArray(), keypoints, descriptors);

    // 將關鍵點轉換為灰度圖像
 
    cv::drawKeypoints(result_g, keypoints, gray, cv::Scalar::all(0)
    ...
    //***********************先看深度值 再看強度值***********************
    int window_size = 3;
    int intensity_threshold = 30; // 假設閾值為10
    int half_window_size = window_size / 2;
    double depth_threshold = 1.5;
...
void LaserProcessingClass::featureExtractionFromSector(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, 
    ...
    for (int i = cloudCurvature.size()-1; i >= 0; i=i-1)
    {
        ...
        if(std::find(picked_points.begin(), picked_points.end(), ind)==picked_points.end()){
            ...
            //一個segment曲率前30大的都push進edge點 //0529
            if (largestPickedNum <= 15 && cloudCurvature[i].value > 0.1 && std::abs(pc_in->points[ind].y) >= 0.5){
                pc_out_edge->push_back(pc_in->points[ind]);
                point_info_count++;
            }
            //若超過25個之後的點曲率還是大於3也是push進edge點  //0529
