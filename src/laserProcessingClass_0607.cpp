...
void LaserProcessingClass::pointcloudtodepth(pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in,
    ...
    cv::Mat result_g, result_G;

    // 濾波後提取邊緣
    GaussianBlur(gray, result_g, cv::Size(5,5), 9, 0); // 高斯濾波
    Laplacian(result_g, result_G, CV_16S, 5, 1, 0);
    convertScaleAbs(result_G, gray);
    ...
    //***********************先看深度值 再看強度值***********************
    int window_size = 3;
    int intensity_threshold = 20; // 假設閾值為10
    int half_window_size = window_size / 2;
...
processImage_surface(depthImage,gray, half_window_size, depth_threshold, intensity_threshold, window_size, planePixels);
    ...
    for (const cv::Point& point : planePixels) {
        ...
        if(depth_value != 0){
            ...
            // if (xx >= 0 && xx < depthImagAssertionError: File path error: ./data//123e.cols && yy >= 0 && yy < depthImage.rows) {
